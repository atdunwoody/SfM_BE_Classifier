...
img_RS = r"Z:\ATD\Drone Data Processing\GIS Processing\Vegetation Filtering Test\Classification_Florian\Test_v1\Test 12 Grid\Inputs\Inputs_Automated\Grid_38\EGI.tif"
...
output_folder = r"Z:\ATD\Drone Data Processing\GIS Processing\Vegetation Filtering Test\Classification_Florian\Test_v1\Test 12 Grid\Results\Results_Prob"
...
# Now use X_scaled for training LDA
rf = LinearDiscriminantAnalysis()

rf2 = rf.fit(X_scaled, y)
...
lda_coefficients = rf.coef_[0]

# Assuming 'bands' is a list of feature names
for b, coef in zip(bands, lda_coefficients):
    print(f'Coefficient for {b}: {coef}')

# You may also want to normalize these coefficients to interpret them more easily
normalized_coefs = lda_coefficients / np.linalg.norm(lda_coefficients)
for b, coef in zip(bands, normalized_coefs):
    print(f'Normalized coefficient for {b}: {coef}')
...
img_as_array = scaler.transform(img_as_array)  # Ensure you transform, not fit_transform

# Predict probabilities
try:
    class_prediction = rf.predict_proba(img_as_array)
except MemoryError:
...
class_probabilities_reshaped = class_prediction.reshape(img.shape[0], img.shape[1], -1)
...
# Apply a threshold to make final class predictions
threshold = 0.5  # Example threshold, adjust as needed
class_prediction = np.argmax(class_probabilities_reshaped, axis=2)
class_prediction_ = (class_probabilities_reshaped[:,:,1] > threshold).astype(np.float16) * mask

#--------------SAVE PROBABILITY RASTER-----------------#
# Save each class probability as a separate band in a raster
output_prob_file = classification_image
prob_driver = gdal.GetDriverByName('GTiff')
prob_raster = prob_driver.Create(output_prob_file, img.shape[1], img.shape[0], class_probabilities_reshaped.shape[2], gdal.GDT_Float32)
prob_raster.SetGeoTransform(img_ds.GetGeoTransform())
prob_raster.SetProjection(img_ds.GetProjection())

for i in range(class_probabilities_reshaped.shape[2]):
    prob_band = prob_raster.GetRasterBand(i+1)
    prob_band.WriteArray(class_probabilities_reshaped[:,:,i])

prob_raster.FlushCache()
prob_raster = None
print('Probability raster saved to: {}'.format(output_prob_file))
